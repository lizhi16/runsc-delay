...
func monitor(cid string) {
	...
	// delay duration
	delay_duration := time.Duration(9000)		//4900 is not enough. 49000 is too much
	wait_duration := time.Duration(5000)
	...
	for {
		...
		addr_out = strings.Replace(addr_out, "'", "", -1)
		addr_out = strings.Replace(addr_out, ",", "", -1)
		addr_out = strings.Replace(addr_out, "\n", "", -1)
...
func judge_delay(access [3]int, index int) bool {
	...
	log.Debugf("[LIZHI] stddev is %f, %f", stddev, dev)
	...
	if (stddev < 175.0 && ratio < 1.85) || (diff < 125 && ratio < 1.85) {
		if mean < 120.0 {
			return false
		}
		return true
