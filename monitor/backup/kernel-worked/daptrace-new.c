...
static void aggregate(void)
{
	...
	mapia_for_each_task(t) {
		...
		mapia_for_each_region(r, t) {
			//lizhi
			int diff = r->vm_end-r->vm_start;
			//if (diff == 4096 || diff == 8192)
			HashNode* pNode = hash_table_lookup(r->vm_start);
			if (pNode == NULL)
				hash_table_insert(r->vm_start, r->nr_accesses); 
			else
				pNode->nValue += r->nr_accesses;
			//printk("[LIZHI] %lx--%lx:[%ld] %d", r->vm_start, r->vm_end, r->vm_end-r->vm_start, r->nr_accesses);
