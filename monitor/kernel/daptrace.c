...
void output_targets_addr(void)
{
        ...
        for (i=0; i<10; i++)
        {
		printk("[LIZHI] %lx: %d\n", targets_addr[i], nr_targets_addr[i]);
	...
	for (; vma; vma = vma->vm_next) {
		...
		if (prev_vma == NULL) {
			...
			*/
			...
			/* lizhi: runsc version
			for (tmp; tmp!=NULL; tmp = tmp->vm_next)
			{
				// runsc version
				if (tmp->vm_file != NULL)
				{
...
{
	// lizhi: judge if the task is alive
	struct pid *id;
	id = find_get_pid(t->pid);
	if(id==NULL)
	{
		pr_err("[LIZHI] pid %d doesn't exist!", t->pid);
		return 0;
	}	
...
static void mapia_split_region_at(struct mapia_region *r, unsigned long sz_r)
{
	...
	//lizhi
        //printk("[LIZHI] mapia_split: %lx --- %lx\n", r->vm_start, r->vm_end);
